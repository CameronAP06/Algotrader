...
def predict_proba(model: LSTMClassifier, X: np.ndarray,
                  seq_len: int = None, batch_size: int = 512) -> np.ndarray:
    """
    Batched sliding-window inference.
    Replaces the old sample-by-sample loop — with 43 800 bars at seq_len=60
    that was ~43 800 individual forward passes; now it is ~86 batches (~500x faster).
    """
    if seq_len is None:
    ...
    with torch.no_grad():
        for start in range(0, n_windows, batch_size):
            batch = torch.from_numpy(np.array(windows[start:start + batch_size])).to(DEVICE)
