...
def fetch_btc_dominance(days: int = 1825) -> pd.DataFrame:
    ...
    try:
        ...
        btc_pct = data["market_cap_percentage"].get("btc", [])
        if not btc_pct:
            raise ValueError("No BTC dominance data")
        ...
        logger.warning(f"BTC dominance fetch failed: {e} — will skip")
        return pd.DataFrame()
...
def merge_alt_data(ohlcv_df: pd.DataFrame, alt_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "btc_dominance" in merged.columns:
        merged["btc_dominance"] = merged["btc_dominance"].fillna(method="ffill").fillna(50.0)
        merged["btc_dom_change_24h"] = merged["btc_dominance"].diff(24)

    if "google_trends_btc" in merged.columns:
        merged["google_trends_btc"] = merged["google_trends_btc"].fillna(method="ffill").fillna(50.0)
